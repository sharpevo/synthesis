...
func TestInstructionDivisionFloat64Execute(t *testing.T) {
	...
	i.Env.Set("var3", &interpreter.Variable{Value: "0"})

	i.Execute("var1", "var2")
	if v, _ := i.Env.Get("var1"); v.(*interpreter.Variable).Value != "0.5" {
		t.Errorf(
			"\nEXPECT: %v\nGET: %v\n",
			"0.50",
	...
	if v, _ := i.Env.Get("var1"); v.(*interpreter.Variable).Value != "0.01500150015" {
		t.Errorf(
			"\nEXPECT: %v\nGET: %v\n",
			"0.01500150015",
			v.(*interpreter.Variable).Value,
	...
	if v, _ := i.Env.Get("var3"); v.(*interpreter.Variable).Value != "0" {
		t.Errorf(
			"\nEXPECT: %v\nGET: %v\n",
			"0",
			v.(*interpreter.Variable).Value,
